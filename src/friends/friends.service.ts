...
export class FriendsService extends TypeOrmCrudService<Friends> {
  ...
  async saveEntity(data: DeepPartial<Friends>[]) {
    return this.friendsRepository.save(this.friendsRepository.create(data));
  }
  ...
  async confirm(user: any, dto: ConfirmDto) {
    const requested: Friends = await this.friendsRepository.findOne({
      where: {
        to_user_id: user.sub,
        from_user_id: dto.id,
      },
  ...
  async getFriends(user: any) {
    const query = await this.friendsRepository.find({
      where: { from_user_id: user.sub, is_accepted: true },
    });
  ...
  async getReceivedRequests(user: any) {
    const query = await this.friendsRepository.find({
      where: [{ to_user_id: user.sub, is_accepted: false, parent_id: null }],
    });
    ...
    for (const i in query) {
      ...
      const u = await UserEntity.findOne({
        where: {
          id:
            query[i].from_user_id == user.sub
              ? query[i].to_user_id
              : query[i].from_user_id,
        },
      });
      friends[i] = {
        user: u,
  ...
  async suggestedFriends(user) {
    ...
    const friendArr = await friend
      .select(['f.to_user_id as to_user_id'])
    ...
    const all = await users
      ...
      .where(
        'u.id NOT IN (:...id) AND u.id != :user AND u.id NOT IN (:...rem)',
        { id: friendId, user: user.sub, rem: removed },
      )
      .getRawMany();
