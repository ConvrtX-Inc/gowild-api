...
export class TicketMessagesController implements CrudController<TicketMessage> {
  ...
  @Post('/:ticket_id')
  public async createTicketMessages(@Param('ticket_id') ticketId: string,@Body() dto: CreateTicketMessageDto, @Request() req) {
    return await this.service.createTicketMessage(ticketId, dto.message ,req.user.sub);
  }
